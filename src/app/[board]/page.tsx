  ...
  if (error || !boardInfo) {
    return (
      <div className="flex flex-col items-center justify-center py-20 text-center">
        ...
        <p className="text-lg text-text-secondary">{error || "Board not found"}</p>
    ...
    <div className="space-y-6">
      ...
      <section className="relative overflow-hidden rounded-2xl border border-border-primary bg-bg-secondary p-6">
        ...
        <div className="relative flex items-center gap-4">
          <div className="flex h-14 w-14 items-center justify-center rounded-xl bg-accent/10 font-display text-lg font-bold text-accent">
            /{boardId}/
          </div>
          <div>
            ...
            <p className="mt-1 text-sm text-text-secondary">{boardInfo.info_outer || boardInfo.info}</p>
            {boardInfo.info && (
        ...
        <div className="relative mt-4 flex flex-wrap items-center gap-2">
          {boardInfo.enable_names && (
            <span className="rounded-full bg-accent/10 px-2 py-0.5 text-[11px] font-medium text-accent">Names</span>
          )}
          {boardInfo.enable_sage && (
            <span className="rounded-full bg-danger/10 px-2 py-0.5 text-[11px] font-medium text-danger">Sage</span>
          )}
          {boardInfo.enable_trips && (
            <span className="rounded-full bg-info/10 px-2 py-0.5 text-[11px] font-medium text-info">Trips</span>
          )}
          {boardInfo.enable_flags && (
            <span className="rounded-full bg-success/10 px-2 py-0.5 text-[11px] font-medium text-success">Flags</span>
          )}
          {boardInfo.enable_icons && (
            <span className="rounded-full bg-sticky-badge/10 px-2 py-0.5 text-[11px] font-medium text-sticky-badge">Icons</span>
          )}
          {boardInfo.enable_likes && (
            <span className="rounded-full bg-success/10 px-2 py-0.5 text-[11px] font-medium text-success">Likes</span>
          )}
          {boardInfo.enable_shield && (
            <span className="rounded-full bg-info/10 px-2 py-0.5 text-[11px] font-medium text-info">Shield</span>
          )}
          {boardInfo.enable_oekaki && (
            <span className="rounded-full bg-accent/10 px-2 py-0.5 text-[11px] font-medium text-accent">Oekaki</span>
          )}
          {boardInfo.enable_thread_tags && (
            <span className="rounded-full bg-sticky-badge/10 px-2 py-0.5 text-[11px] font-medium text-sticky-badge">Tags</span>
          )}
          {boardInfo.enable_dices && (
            <span className="rounded-full bg-info/10 px-2 py-0.5 text-[11px] font-medium text-info">Dice</span>
          )}
        </div>

        <div className="relative mt-3 flex flex-wrap gap-3 text-xs text-text-muted">
